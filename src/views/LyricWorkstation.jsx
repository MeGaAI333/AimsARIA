...
export default function LyricWorkstation({ apiKey }) {
  ...
  const generate = async () => {
    ...
    try {
      ...
      setGenerated(data.content?.[0]?.text || (data.error ? `⚠️ ${data.error.message}` : "Error generating content."));
              ...
              <div style={{ flex:1, overflowY:"auto", padding:24 }}>
                ...
                {loading && (
                  <div style={{ textAlign:"center", marginTop:80 }}>
                    <div style={{ fontSize:32, marginBottom:16, animation:"aiPulse 1s infinite" }}>🎵</div>
                    <div style={{ fontSize:14, color:GREEN, fontWeight:700 }}>LYRIC is writing your {contentType}…</div>
                    <div style={{ fontSize:12, color:C.textMuted, marginTop:8 }}>Crafting industry-fluent content for {industry}</div>
                  </div>
                )}
                {generated && !loading && (
                  <div>
                    ...
                    <div style={{ background:C.card, border:`1px solid ${GREEN}25`, borderRadius:10, padding:24 }}>
                      <pre style={{ margin:0, fontFamily:"inherit", fontSize:13, color:C.textPrimary, lineHeight:1.7, whiteSpace:"pre-wrap", wordBreak:"break-word" }}>{generated}</pre>
                    </div>
                  </div>
                )}
              </div>
