...
const AIMS_SERVICES = [
  { id:"aria",   name:"ARIA",   icon:"🔵", desc:"AI inbound recovery — responds to cold leads 24/7, handles objections, qualifies prospects" },
  { id:"melody", name:"MELODY", icon:"🟣", desc:"AI outbound closing — books appointments from warm leads that ARIA hands off" },
...
const BLANK = {
  ...
  services_selected:[], plan_tier:"Pro ($997/mo)",
...
function Field({ label, k, form, set, type="text", placeholder="" }) {
  return (
...
function Textarea({ label, k, form, set, placeholder="", rows=3 }) {
  return (
    <div style={{ marginBottom:14 }}>
      <label style={{ display:"block", fontSize:11, fontWeight:700, color:C.textSecondary, textTransform:"uppercase", letterSpacing:0.5, marginBottom:5 }}>{label}</label>
...
function SelectField({ label, k, form, set, options, placeholder="" }) {
  return (
    <div style={{ marginBottom:14 }}>
      <label style={{ display:"block", fontSize:11, fontWeight:700, color:C.textSecondary, textTransform:"uppercase", letterSpacing:0.5, marginBottom:5 }}>{label}</label>
...
function SectionHead({ title }) {
  return <div style={{ fontSize:10, fontWeight:800, color:C.textMuted, textTransform:"uppercase", letterSpacing:1, marginBottom:14 }}>{title}</div>;
}

// ─── Wizard ─────────────────────────────────────────────────────────────────

function OnboardingWizard({ orgId, onComplete }) {
  ...
  useEffect(() => {
    ...
    setLoading(true); setSaved(false); setDone(false);
    getOnboarding(orgId)
  ...
  const persist = async (overrides = {}) => {
    ...
    // Strip empty date strings so Postgres doesn't reject them
  ...
  const complete = async () => {
    const newStatus = ["Not Started","In Progress"].includes(form.status) ? "Config Complete" : form.status;
    await persist({ status: newStatus });
    // Auto-generate onboarding tasks per service
    const biz = form.business_name || orgId;
    const taskSets = {
      aria: [
        `[${biz}] ARIA — Connect lead sources & form integrations`,
        `[${biz}] ARIA — Upload cold lead list`,
        `[${biz}] ARIA — Configure brand voice & objection library`,
        `[${biz}] ARIA — Get client sign-off on touch sequence`,
        `[${biz}] ARIA — Go live`,
      ],
      melody: [
        `[${biz}] MELODY — Connect calendar / booking system`,
        `[${biz}] MELODY — Configure closing sequences`,
        `[${biz}] MELODY — Test handoff flow from ARIA`,
      ],
      lyric: [
        `[${biz}] LYRIC — Complete Client Profile (voice & brand)`,
        `[${biz}] LYRIC — Draft first month content calendar`,
        `[${biz}] LYRIC — Connect Buffer or scheduling tool`,
      ],
      muse: [
        `[${biz}] MUSE — Connect Google Business & Facebook`,
        `[${biz}] MUSE — Audit existing reviews`,
        `[${biz}] MUSE — Build response library`,
      ],
    };
  ...
  const renderStep = () => {
    switch (step) {
      ...
      case 1:
        return (
          <>
            ...
            <Textarea label="Primary Pain / Challenge (what made them sign up?)" k="primary_pain"
            ...
            {form.current_crm === "Other" && (
              <Field label="CRM Name (specify)" k="crm_other" placeholder="Enter CRM name" {...shared} />
            )}
            <Textarea label="Additional Notes on Current State" k="situation_notes"
              placeholder="e.g. Has 2 CSRs, overwhelmed in summer. Tracks estimates in email. Runs ServiceTitan but rarely uses it for follow-up." rows={2} {...shared} />
          </>
        );
      case 2:
        return (
          <>
            <SectionHead title="Select AIMS Services for This Client" />
            ...
            <div style={{ marginBottom:14 }}>
              ...
              <div style={{ display:"flex", gap:10 }}>
                {["Pro ($997/mo)","Elite (Multi-location)"].map(t => (
      ...
      case 3:
        return (
          <>
            <Chips label="Where Do Their Leads Come From?" k="lead_sources" options={LEAD_SOURCES} {...shared} />
            <div style={{ background:C.surface, borderRadius:10, padding:16, marginBottom:14 }}>
              ...
              {form.has_cold_list && (
                <Field label="Estimated Cold List Size (contacts)" k="cold_list_size" placeholder="e.g. 400" {...shared} />
              )}
            </div>
            <Textarea label="Notes on Lead Sources" k="lead_source_notes"
              placeholder="e.g. $5K/mo Google Ads, also uses Angi but frustrated with lead quality. Website form is primary." rows={2} {...shared} />
          </>
        );
      case 4:
        return (
          <>
            <SectionHead title="ARIA & MELODY Voice Configuration" />
            <Field label='AI Agent Display Name (how ARIA signs messages)' k="agent_name" placeholder='e.g. "Aria with Acme HVAC" or just "Aria"' {...shared} />
            ...
            <Toggle label="Use emojis in outreach messages" desc="Appropriate for MedSpa, consumer brands; generally avoid for legal, financial, commercial" k="emoji_use" {...shared} />
            <Textarea label="Certifications / Credentials to Highlight" k="certifications"
              placeholder="e.g. NATE-certified, 10-year labor warranty, A+ BBB, Licensed & Insured, 200+ 5-star reviews" rows={2} {...shared} />
            <Textarea label="Top 3 Objections (what prospects say when they don't book)" k="top_objections"
              placeholder="1. 'I'm getting other quotes'  2. 'I need to talk to my spouse'  3. 'What's the price before you come out?'" rows={3} {...shared} />
            <Textarea label="Language / Topics to NEVER Use" k="phrases_to_avoid"
              placeholder="e.g. No price comparisons with competitors, no political content, never use high-pressure language" rows={2} {...shared} />
          </>
        );
      case 5:
        return (
          <>
            ...
            <Chips label="Active Social Media Platforms" k="social_platforms" options={SOCIAL_PLATS} {...shared} />
            ...
            <div style={{ background:C.surface, borderRadius:10, padding:16, marginBottom:14 }}>
              ...
              {form.has_email_list && (
                <Field label="List Size (contacts)" k="email_list_size" placeholder="e.g. 1,200" {...shared} />
              )}
      ...
      case 6:
        return (
          <>
            <SectionHead title="Integrations & Workflow" />
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"0 16px" }}>
              <SelectField label="CRM to Connect" k="crm_integration" options={CRMS} placeholder="Select CRM…" {...shared} />
              <SelectField label="Calendar / Booking System" k="calendar_system" options={CALENDAR_OPTS} placeholder="Select calendar…" {...shared} />
            </div>
            <Field label="Direct Booking Link (if client self-schedules)" k="booking_link" placeholder="https://acmehvac.com/book" {...shared} />
            <Field label="Content / Messaging Approval Contact" k="approval_contact" placeholder="Name & email — who approves outreach before it goes live" {...shared} />
            <Field label="Lead Notification Contact (who receives MELODY handoffs)" k="leads_notification_contact" placeholder="Name, email, or phone" {...shared} />
            <Textarea label="Integration Notes" k="integration_notes"
              placeholder="e.g. ServiceTitan export is manual CSV monthly. Calendar is Google, booking goes through CSR. Wants SMS-only, no email." rows={3} {...shared} />
          </>
        );
      case 7:
        return (
          <>
            <SectionHead title="Go-Live Planning" />
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"0 16px" }}>
              <Field label="Target Go-Live Date" k="target_start_date" type="date" {...shared} />
              <SelectField label="Onboarding Status" k="status" options={STATUS_OPTS} {...shared} />
            </div>
            <div style={{ background:C.surface, borderRadius:10, padding:16, marginBottom:14 }}>
              <Toggle label="Kickoff call has been scheduled" k="kickoff_scheduled" {...shared} />
              {form.kickoff_scheduled && (
                <Field label="Kickoff Call Date & Time" k="kickoff_date" type="datetime-local" {...shared} />
              )}
            </div>
            <Textarea label="Internal Notes for AIMS Team" k="notes"
              placeholder="Special requirements, commitments made during the sale, client expectations to manage, anything else the team needs to know." rows={5} {...shared} />
  ...
  return (
    <div style={{ maxWidth:780 }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10, paddingBottom:32 }}>
        ...
        {done && <span style={{ fontSize:12, color:C.green, fontWeight:700 }}>✓ Complete — Tasks Generated</span>}
  ...
  if (role !== "admin") {
    return (
      <div style={{ padding:"28px 32px", overflowY:"auto", height:"100%", boxSizing:"border-box" }}>
        <h2 style={{ margin:"0 0 4px", fontSize:20, fontWeight:800, color:C.textPrimary }}>Client Onboarding</h2>
        <p style={{ margin:"0 0 24px", fontSize:13, color:C.textSecondary }}>Complete each step so we can configure your AIMS services. Your progress is saved as you go.</p>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"28px 32px" }}>
        {!selectedOrg ? (
          ...
          <>
            ...
            <OnboardingWizard key={selectedOrg} orgId={selectedOrg} onComplete={refreshList} />
          </>
        )}
      </div>
    </div>
